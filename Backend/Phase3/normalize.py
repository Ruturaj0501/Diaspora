...
def load_place_aliases(filepath="Phase3/alias_corpus.json"):
    ...
    try:
        ...
        for canonical_name, aliases in data.get("places", {}).items():
            flat_aliases[canonical_name.lower()] = canonical_name
            for alias in aliases:
                flat_aliases[alias.lower().strip()] = canonical_name
                
        return flat_aliases
    except FileNotFoundError:
        print(f"[ERROR] '{filepath}' not found. Place resolution will fail.")
        return {}
...
def normalize_place(place_str):
    """Goal 2: Place Resolution API (Fuzzy Matching against alias_corpus.json)"""
    clean_place = place_str.lower().strip()
    
    if not FLAT_ALIASES:
        return f"UNRESOLVED: {place_str} (Corpus missing)"
...
def main():
    ...
    try:
        ...
        print("[ERROR] Phase2/phase2_output.json not found. Run Phase 2 first.")
        return
    ...
    with open("Phase3/phase3_output.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4)

    print("\n[SUCCESS] Phase 3 completed.")
    print("[SUCCESS] Output saved to Phase3/phase3_output.json")
